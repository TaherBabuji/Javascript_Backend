...
const loginUser = asyncHandler(async (req, res) => {
  // will take registered email, password, username from user
  // will authorize user input that user is not provide any empty field
  // will validate the email that it is proper
  // Will check password that it matches properly
  // validate user's input from the data in the db that it matches or not
  // will generate refresh token and excess token
  // send it to user using secured cookies
  // provide user a response that they are logged in successfully
...
const logoutUser = asyncHandler(async (req, res) => {
  ...
  if (!updatedUser) {
    throw new ApiError(404, "User not found while deleting refresh token");
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverImage.url) {
    throw new ApiError(400, "Error while uploading user coverImage");
  }
